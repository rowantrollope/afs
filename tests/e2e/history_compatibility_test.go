...
func TestHistoryCompatibilityCLIAndHTTPProcess(t *testing.T) {
	...
	c.run(nil, "versioning", "compatible", "--mode", "all")
	...
	if err := json.Unmarshal(c.run(nil, "--json", "file", "history", "compatible", "file.txt", "--order", "asc", "--limit", "1"), &first); err != nil {
		t.Fatal(err)
	}
	...
	if err := json.Unmarshal(c.run(nil, "--json", "file", "history", "compatible", "file.txt", "--order", "asc", "--limit", "1", "--cursor", first.NextCursor), &second); err != nil {
		t.Fatal(err)
	}
	if len(second.Lineages) != 1 || second.Lineages[0].Versions[0].Ordinal <= version.Ordinal {
		t.Fatalf("second history page: %+v", second)
	}
	if shown := c.run(nil, "--json", "file", "show", "compatible", "file.txt", "--version", version.VersionID); !bytes.Contains(shown, []byte(`"content":"old\n"`)) {
		t.Fatalf("show: %s", shown)
	}
	diff := c.run(nil, "--json", "file", "diff", "compatible", "file.txt", "--from-version", version.VersionID, "--to-ref", "working-copy")
	if !bytes.Contains(diff, []byte("-old")) || !bytes.Contains(diff, []byte("+new")) {
		t.Fatalf("CLI diff: %s", diff)
	}
	c.run(nil, "file", "restore", "compatible", "file.txt", "--version", version.VersionID)
	if got := c.published("compatible", "file.txt"); string(got) != "old\n" {
		t.Fatalf("CLI restore: %q", got)
	}
	c.removePublished("compatible", "file.txt")
	c.closeWriter("compatible")
	c.run(nil, "file", "undelete", "compatible", "file.txt")
	if got := c.published("compatible", "file.txt"); string(got) != "old\n" {
	...
	defer cancel()
	process := c.command(ctx, "--json", "serve", "--listen", "127.0.0.1:0", "--database-id", "selected", "--allow-origin", "http://localhost:5173")
	...
	var compact filehistory.Page
	if err := json.Unmarshal(c.run(nil, "--json", "history", "compatible", "file.txt"), &compact); err != nil || compact.FileID != version.FileID {
		t.Fatalf("compact interface changed: %+v %v", compact, err)
	}
	c.run(nil, "versioning", "compatible", "--mode", "off")
