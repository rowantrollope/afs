...
func TestSharedFUSEMountEnforcesPermissions(t *testing.T) {
	for _, shared := range []bool{false, true} {
		for _, readonly := range []bool{false, true} {
			opts := mountOptions(&Options{AllowOther: shared, ReadOnly: readonly})
			if opts.AllowOther != shared || slices.Contains(opts.Options, "default_permissions") != shared {
				t.Fatalf("shared=%v: kernel permissions missing or private behavior changed: %+v", shared, opts.MountOptions)
			}
			if slices.Contains(opts.Options, "ro") != readonly || !opts.NullPermissions {
				t.Fatalf("readonly=%v: read-only or zero-mode handling changed: %+v", readonly, opts)
			}
		}
	}
}
