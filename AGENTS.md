...
- Control-plane scope is self-managed Redis connections, a bootstrap team token, and named trusted-administrator API keys. Keys support expiry, usage tracking and API revocation; they do not enforce workspace isolation or revoke previously issued Redis credentials. Connection profiles, administrator keys and default selection use private SQLite locally or shared Postgres through AFS_METADATA_URL; reuse the same Redis storage engine per connection. The control plane starts without Redis and keeps metadata/auth independent of managed Redis availability. Hosted mode requires a token and Postgres, refreshes routing across instances and rejects stale registry writes. No Cloud provisioning, search, hosted MCP, templates, or duplicated workspace catalog.
