...
function changeOperationLabel(
  row: Pick<
    HistoryTableRow,
    "historyType" | "kind" | "op" | "prevHash" | "prevPath"
  >,
): string {
...
function lifecycleOperationLabel(kind?: string, op?: string): string {
  switch (`${kind ?? ""}.${op ?? ""}`) {
    ...
    default:
      return (
        [formatToken(kind), formatToken(op)].filter(Boolean).join(" ") ||
        "Event"
      );
  }
}
...
export function ChangesTable({
  ...
  const columns = useMemo(() => {
    const showWorkspaceContext = rows.some(
      (row) =>
        Boolean(row.workspaceName?.trim()) || Boolean(row.databaseName?.trim()),
    );
    ...
    return [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Typography.Body component="span">
            {changeOperationLabel(row.original)}
          </Typography.Body>
        ),
      },
      {
        ...
        ? [
            {
              ...
              size: 170,
              enableSorting: false,
              cell: ({ row }: { row: { original: HistoryTableRow } }) => (
                <S.Stack>
                  <S.SingleLineText
                    title={
        ...
        : []),
      {
        ...
        cell: ({ row }) => {
          ...
          const color =
            delta > 0 ? "#16a34a" : delta < 0 ? "#dc2626" : undefined;
          return (
            <Typography.Body
              component="span"
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          const tooltip = [label, actor, agentId, sessionId]
            .filter(Boolean)
            .join(" · ");
          return (
...
function lifecycleSecondary(row: HistoryTableRow, detail: string): string {
  return [
    ...
    .filter((part): part is string =>
      Boolean(part && part.trim() !== "" && part !== detail),
    )
    .join(" · ");
}
