...
import {
  computeChangelogTotals,
  formatChangelogBytes,
} from "../foundation/changelog-utils";
...
function ActivityPage() {
  ...
  const canPageChangelogNext =
    Boolean(changelogQuery.data?.nextCursor) &&
    changelogEntries.length === ACTIVITY_PAGE_SIZE;
  const canPageEventsNext =
    Boolean(eventsQuery.data?.nextCursor) &&
    eventRows.length === ACTIVITY_PAGE_SIZE;
    ...
    <PageStack>
      {unavailableDatabases.length > 0 ? (
        <NoticeCard $tone="warning" role="status">
          ...
          <NoticeBody>
            {view === "changes" ? "Changelog" : "History"} below are partial
            while these databases are disconnected:{" "}
        ...
        <SectionGrid>
          <SectionCard $span={12}>
            <SectionHeader>
              ...
              <HeaderSummary>
                {changelogQuery.isLoading ? (
                  ...
                  <>
                    Showing <strong>{changelogEntries.length}</strong> changes
                    on this page · <strong>{changelogTotals.added}</strong>{" "}
                    added · <strong>{changelogTotals.modified}</strong> modified
                    · <strong>{changelogTotals.deleted}</strong> deleted ·{" "}
                    <PositiveDelta>
                      +{formatChangelogBytes(changelogTotals.bytesAdded)}
                    </PositiveDelta>
                    {" / "}
                    <NegativeDelta>
                      −{formatChangelogBytes(changelogTotals.bytesRemoved)}
                    </NegativeDelta>
                  </>
                ) : (
                  "No changes yet"
            ...
            (changelogCursors.length > 0 || canPageChangelogNext) ? (
              <PaginationControls
                onPrevious={() =>
                  setChangelogCursors((cursors) => cursors.slice(0, -1))
                }
                onNext={() => {
      ...
      {view === "events" ? (
        <>
          ...
          (eventCursors.length > 0 || canPageEventsNext) ? (
            <PaginationControls
              onPrevious={() =>
                setEventCursors((cursors) => cursors.slice(0, -1))
              }
              onNext={() => {
