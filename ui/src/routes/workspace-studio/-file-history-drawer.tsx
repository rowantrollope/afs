...
export function FileHistoryDrawer({
  ...
  const mergedHistory = useMemo(() => {
    ...
    return {
      ...
      order: pages[0].order,
      lineages: order
        .map((fileId) => lineages.get(fileId))
        .filter(Boolean) as AFSFileHistoryLineage[],
      nextCursor,
  ...
  useEffect(() => {
    ...
    if (targetVersionId) {
      const exact = flattenedVersions.find(
        (entry) => entry.version.versionId === targetVersionId,
      );
      if (exact) {
  ...
  const selected = useMemo<SelectedVersion | null>(
    () =>
      flattenedVersions.find(
        (entry) => versionSelectionKey(entry.version) === selectedKey,
      ) ?? null,
  ...
  const canDiff =
    displayedVersion != null && displayedVersion.kind !== "tombstone";
  const canRestore =
    editable &&
    displayedVersion != null &&
    displayedVersion.kind !== "tombstone";
  const canUndelete =
    editable &&
    displayedVersion != null &&
    (displayedVersion.kind === "tombstone" ||
      displayedLineage?.state === "deleted");
  const busy =
  ...
  async function handleRestore() {
    ...
    setActionMessage(
      `Restored ${response.restoredFromVersionId} into the working copy.`,
    );
  }
  ...
  async function handleUndelete() {
    ...
    setActionMessage(
      `Undeleted ${response.undeletedFromVersionId} into the working copy.`,
    );
  }
  ...
  function handlePathActivitySelect(entry: AFSChangelogEntry) {
    ...
    const exact = flattenedVersions.find(
      (item) => item.version.versionId === entry.versionId,
    );
    if (exact) {
    ...
    <Overlay onClick={onClose}>
      ...
      >
        <Header>
          ...
          <Button size="small" variant="secondary-fill" onClick={onClose}>
            Close
          </Button>
        </Header>
          ...
          <Sidebar>
            <SidebarSection>
              <SidebarSectionHeader>
                ...
                <SectionCaption>
                  Recent file operations for this path.
                </SectionCaption>
              </SidebarSectionHeader>
              {pathActivityQuery.isLoading ? (
                <EmptyMessage>Loading path activity…</EmptyMessage>
              ) : null}
              {pathActivityQuery.isError ? (
                <EmptyMessage role="alert">
                  {pathActivityQuery.error instanceof Error
                    ? pathActivityQuery.error.message
                    : "Unable to load path activity."}
                </EmptyMessage>
              ) : null}
              {!pathActivityQuery.isLoading &&
              !pathActivityQuery.isError &&
              (pathActivityQuery.data?.entries.length ?? 0) === 0 ? (
                <EmptyMessage>
                  No recent path activity for this file.
                </EmptyMessage>
              ) : null}
                ...
                return (
                  ...
                  >
                    ...
                    <VersionMeta>
                      ...
                      <span>
                        {entry.versionId
                          ? `v${linkedOrdinal(entry.versionId, flattenedVersions) ?? "?"}`
                          : "event only"}
                      </span>
                    </VersionMeta>
                  </ActivityButton>
            ...
            <SidebarSection>
              <SidebarSectionHeader>
                ...
                <SectionCaption>
                  Ordered versions for this file lineage.
                </SectionCaption>
              </SidebarSectionHeader>
              {historyQuery.isLoading ? (
                <EmptyMessage>Loading file history…</EmptyMessage>
              ) : null}
              ...
              {!historyQuery.isLoading &&
              !historyQuery.isError &&
              (mergedHistory?.lineages.length ?? 0) === 0 ? (
                <EmptyMessage>
                  No tracked history yet for this path.
                </EmptyMessage>
              ) : null}
              {mergedHistory?.lineages.map((lineage) => (
                <LineageCard key={lineage.fileId}>
                  ...
                  <VersionList>
                    {lineage.versions.map((version) => {
                      const selectedNow =
                        versionSelectionKey(version) === selectedKey;
                      return (
                        <VersionButton
              ...
              {mergedHistory?.nextCursor ? (
                <LoadMoreWrap>
                  ...
                  {loadMoreError ? (
                    <InlineError role="alert">{loadMoreError}</InlineError>
                  ) : null}
                </LoadMoreWrap>
              ) : null}
            </SidebarSection>
            ...
            {displayedVersion ? (
              <>
                <DetailHeader>
                  <DetailMeta>
                    ...
                    <DetailTags>
                      ...
                      {displayedVersion.sizeBytes != null ? (
                        <Tag>{formatBytes(displayedVersion.sizeBytes)}</Tag>
                      ) : null}
                    </DetailTags>
                  </DetailMeta>
                  <DetailActions>
                ...
                {panelMode === "diff" ? (
                    ...
                    {diffMutation.isError ? (
                      <InlineError role="alert">
                        {diffMutation.error instanceof Error
                          ...
                          : "Unable to diff this version."}
                      </InlineError>
                    ) : null}
                    {diffMutation.data?.binary ? (
                      <EmptyMessage>
                        Binary diff. A textual unified diff is not available for
                        this version.
                      </EmptyMessage>
                    ) : (
                  ...
                  <InlineError role="alert">
                    {selectedVersionContentQuery.error instanceof Error
                      ? selectedVersionContentQuery.error.message
                      : "Unable to load version content."}
                  </InlineError>
                ) : content?.kind === "tombstone" ? (
                  <EmptyMessage>
                    This version is a tombstone. Use undelete to bring the
                    deleted lineage back.
                  </EmptyMessage>
                ) : content?.binary ? (
                  <EmptyMessage>
                    Binary file. History metadata is available, but the content
              ...
              <EmptyMessage>
                Select a version to inspect its content or compare it to head.
              </EmptyMessage>
            )}
          </DetailPane>
        </Body>
...
function versionSelectionKey(
  version: Pick<AFSFileVersion, "fileId" | "ordinal">,
) {
...
function linkedOrdinal(
  ...
  return flattenedVersions.find(
    (entry) => entry.version.versionId === versionId,
  )?.version.ordinal;
}
...
const VersionButton = styled.button<{ $active: boolean }>`
  ...
  border: 1px solid
    ${({ $active }) =>
      $active ? "var(--afs-selection-border)" : "var(--afs-line)"};
  background: ${({ $active }) =>
    $active ? "var(--afs-selection-bg)" : "var(--afs-panel-strong)"};
  color: ${({ $active }) =>
    $active ? "var(--afs-selection-text)" : "var(--afs-ink)"};
  padding: 10px 12px;
  cursor: pointer;
  transition:
  ...
  &:hover {
    ...
    background: ${({ $active }) =>
      $active ? "var(--afs-selection-bg)" : "var(--afs-selection-hover-bg)"};
    color: ${({ $active }) =>
      $active ? "var(--afs-selection-text)" : "var(--afs-selection-hover-ink)"};
  }
`;

const ActivityButton = styled(VersionButton)`
  background: ${({ $active }) =>
    $active ? "var(--afs-selection-bg)" : "rgba(255, 255, 255, 0.45)"};
`;
