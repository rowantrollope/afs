...
export function CheckpointsTab({
  ...
  const [selectedSavepoint, setSelectedSavepoint] =
    useState<AFSSavepoint | null>(null);
  const [expandedSavepointId, setExpandedSavepointId] = useState<string | null>(
    null,
  );
  ...
  return (
    <>
      {createSavepoint.error || restoreSavepoint.error ? (
        <DialogError role="alert">
          {(createSavepoint.error ?? restoreSavepoint.error)?.message}
        </DialogError>
      ) : null}
      {workspace.capabilities.createCheckpoint ? (
        <SectionGrid>
          <SectionCard $span={12}>
            ...
            <DialogBody>
              Capture the current published Redis state. Files that a client has
              not synchronized are not included.
            </DialogBody>
            <FormGrid
              onSubmit={(event) => {
            ...
            {filteredSavepoints.length === 0 ? (
              ...
              <S.TableCard>
                <CheckpointHistoryViewport>
                  <CheckpointHistoryTable aria-label="Checkpoint history">
                    <thead>
                      <tr>
                    ...
                    <tbody>
                      {filteredSavepoints.map((savepoint) => {
                        const isActive = isActiveCheckpoint(
                          workspace,
                          savepoint,
                        );
                        ...
                        return (
                          <Fragment key={savepoint.id}>
                            ...
                            >
                              <CheckpointCell>
                                {shortDateTime(savepoint.createdAt)}
                              </CheckpointCell>
                              <CheckpointCell>
                                <CheckpointNameCell>
                                  <ExpandGlyph
                              ...
                              <CheckpointCell>
                                <S.Stack>
                                  ...
                                  <Typography.Body
                                    ...
                                    component="span"
                                  >
                                    {savepoint.fileCount} files ·{" "}
                                    {savepoint.folderCount} folders
                                  </Typography.Body>
                                </S.Stack>
                              </CheckpointCell>
                              <CheckpointCell
      ...
      {selectedSavepoint ? (
        <CheckpointDetailDialog
          ...
          compareDisabled={workspace.liveRootAvailable === false}
          restoreDisabled={
...
function isActiveCheckpoint(
  ...
  return (
    workspace.liveRootAvailable !== false &&
    getActiveWorkspaceView(workspace) === "head" &&
...
function CheckpointExpandedPanel({
  ...
  const typeLabel = savepoint.kind
    ? formatCheckpointKind(savepoint.kind)
    : "Checkpoint";
  const sourceLabel = savepoint.source
    ? formatCheckpointSource(savepoint.source)
    : "Unknown";
  ...
  return (
    <ExpandedPanelStack>
      <ExpandedPanelHeader>
        ...
        <CheckpointBadgeRow>
          {isActive ? (
            <ActiveCheckpointBadge>Active</ActiveCheckpointBadge>
          ) : null}
      ...
      <ExpandedDetailGrid>
        <DetailField>
          ...
          <DetailValue $mono title={savepoint.id}>
            {savepoint.id}
          </DetailValue>
        </DetailField>
        <DetailField>
        ...
        <DetailField>
          ...
          <DetailValue>
            {savepoint.fileCount.toLocaleString()} files ·{" "}
            {savepoint.folderCount.toLocaleString()} folders ·{" "}
            {savepoint.sizeLabel}
          </DetailValue>
        </DetailField>
      </ExpandedDetailGrid>
...
function CheckpointDiffDialog({
  ...
  const liveAvailable = workspace.liveRootAvailable !== false;
  ...
  const title =
    mode === "restore"
      ? `Restore ${savepoint.name}`
      : `Compare ${savepoint.name}`;
  const body = !liveAvailable
    ? "The live tree is missing. Restoring this checkpoint will recreate the workspace tree from its saved files."
    : mode === "restore"
  ...
  return (
    <DialogOverlay onClick={onClose}>
      <CheckpointDialogCard onClick={(event) => event.stopPropagation()}>
        ...
        {!liveAvailable ? null : diffQuery.isLoading ? (
          ...
          {mode === "restore" ? (
            <Button
              ...
              disabled={
                restoreDisabled ||
                restorePending ||
                (liveAvailable && (diffQuery.isLoading || diffQuery.isError))
              }
              onClick={onRestore}
            >
    ...
    <DiffStack>
      <DiffSummaryGrid>
        ...
        <DiffStat>
          ...
          <DiffStatValue>
            +{formatBytesForDiff(diff.summary.bytesAdded)} / -
            {formatBytesForDiff(diff.summary.bytesRemoved)}
          </DiffStatValue>
        </DiffStat>
      </DiffSummaryGrid>
      ...
      {diff.entries.length === 0 ? (
        ...
        <DiffList>
          {diff.entries.map((entry) => (
            <DiffRowShell
              key={`${entry.op}:${entry.previousPath ?? ""}:${entry.path}`}
            >
              <DiffRow>
                <DiffOpBadge $op={entry.op}>
                  {formatDiffOp(entry.op)}
                </DiffOpBadge>
                <DiffPathStack>
                  <DiffPath title={diffEntryPathTitle(entry)}>
                    {diffEntryPath(entry)}
                  </DiffPath>
...
function InlineTextDiff({ entry }: { entry: AFSDiffEntry }) {
  ...
  if (!textDiff.available) {
    return textDiff.skippedReason ? (
      <DiffInlineBlock>
        <DiffInlineNotice>
          Text diff not shown: {textDiff.skippedReason}
        </DiffInlineNotice>
      </DiffInlineBlock>
    ) : null;
  }
  ...
  return (
    <DiffInlineBlock>
      {hunks.map((hunk, hunkIndex) => (
        <DiffHunk
          key={`${entry.path}:${hunk.oldStart}:${hunk.newStart}:${hunkIndex}`}
        >
          <DiffHunkHeader>
...
function diffEntryMeta(entry: AFSDiffEntry) {
  ...
  const before =
    entry.previousSizeBytes == null
      ? ""
      : formatBytesForDiff(entry.previousSizeBytes);
  const after =
    entry.sizeBytes == null ? "" : formatBytesForDiff(entry.sizeBytes);
...
function formatDiffDelta(value?: number) {
  ...
  return value > 0
    ? `+${formatBytesForDiff(value)}`
    : `-${formatBytesForDiff(Math.abs(value))}`;
}
...
const CheckpointSummaryRow = styled.tr<{ $expanded: boolean }>`
  cursor: pointer;
  background: ${({ $expanded }) =>
    $expanded ? "var(--afs-panel)" : "transparent"};
...
const DetailValue = styled.span<{ $mono?: boolean }>`
  ...
  font-family: ${({ $mono }) =>
    $mono
      ? "var(--afs-mono, ui-monospace, SFMono-Regular, Menlo, Consolas, monospace)"
      : "inherit"};
