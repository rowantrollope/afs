...
export const NoticeCard = styled(SurfaceCard)<{
...
export const TabButton = styled.button<{ $active?: boolean }>`
  ...
  border: 1px solid
    ${({ $active }) =>
      $active ? "var(--afs-selection-border)" : "transparent"};
  background: ${({ $active }) =>
    $active ? "var(--afs-selection-bg)" : "transparent"};
  color: ${({ $active }) =>
    $active ? "var(--afs-selection-text)" : "var(--afs-muted, #71717a)"};
  padding: 9px 16px;
  border-radius: 10px;
  font-size: 13px;
  font-weight: 700;
  ...
  [data-skin="situation-room"] && {
    ...
    background: ${({ $active }) =>
      $active ? "var(--afs-selection-bg)" : "transparent"};
    color: ${({ $active }) =>
      $active ? "var(--afs-selection-text)" : "var(--afs-ink-dim)"};
    font-weight: var(--afs-fw-regular);
    letter-spacing: var(--afs-tr-caps);
    text-transform: uppercase;
    font-size: var(--afs-fz-xs);
    ...
    &:hover {
      ...
      color: ${({ $active }) =>
        $active
          ? "var(--afs-selection-text)"
          : "var(--afs-selection-hover-ink)"};
      background: ${({ $active }) =>
        $active ? "var(--afs-selection-bg)" : "var(--afs-selection-hover-bg)"};
    }
  }
...
export const FileButton = styled.button<{ $active?: boolean }>`
  ...
  [data-skin="situation-room"] && {
    ...
    border-left: var(--afs-selection-indicator-width) solid
      ${({ $active }) =>
        $active ? "var(--afs-selection-indicator)" : "transparent"};
    background: ${({ $active }) =>
      $active ? "var(--afs-selection-bg)" : "var(--afs-bg-1)"};
    color: ${({ $active }) =>
      $active ? "var(--afs-selection-text)" : "var(--afs-ink-dim)"};
    ...
    <ActivityList>
      {props.events.map((event) => (
        <ActivityCard key={event.id}>
          <Typography.Body component="span">{event.title}</Typography.Body>
