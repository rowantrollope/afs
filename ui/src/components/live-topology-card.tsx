  ...
  ${({ $i, $presence }) =>
    $presence === "present"
      ...
      : css`
          ...
          animation: ${$presence === "exiting" ? popOut : popIn}
            ${TOPOLOGY_NODE_EXIT_MS}ms cubic-bezier(0.2, 0.8, 0.2, 1) forwards;
          animation-delay: ${$presence === "entering"
            ? `${Math.min($i, 6) * 0.04}s`
            : "0s"};
        `}
`;
    ...
    $highlighted
      ? css`
          ...
          box-shadow:
            ...
            inset var(--afs-selection-indicator-width) 0 0
              var(--afs-selection-indicator),
...
const NodeIconBox = styled.div<{ $active?: boolean }>`
  ...
  color: ${({ $active }) =>
    $active ? "var(--afs-ok, #22c55e)" : "var(--afs-accent, #dc2626)"};
...
/* ---- Workspace nodes ---- */
const WorkspaceNode = styled.button<{
  $i: number;
  $presence: NodePresence;
  $highlighted?: boolean;
}>`
...
const LiveStatusDot = styled.span`
  ...
  box-shadow: 0 0 8px
    color-mix(in srgb, var(--afs-ok, #22c55e) 70%, transparent);
  flex-shrink: 0;
`;
...
const DashedPolyline = styled.polyline<{
  $color: string;
  $highlighted?: boolean;
  $animated: boolean;
}>`
  ...
  filter: ${({ $highlighted }) =>
    $highlighted ? "drop-shadow(0 0 4px currentColor)" : "none"};
  animation: ${({ $animated }) =>
    $animated
      ? css`
...
): row is AnimatedTopologyItem<T> & {
  presence: Exclude<NodePresence, "exiting">;
} {
...
function useAnimatedTopologyItems<T>(
  ...
  useEffect(() => {
    ...
    setRendered((previous) => {
      ...
      const next: AnimatedTopologyItem<T>[] = items.map((item) => {
        const id = getId(item);
    ...
    const exitTimer = window.setTimeout(() => {
      setRendered((current) =>
        current.filter(
          (row) => incomingIds.has(row.id) || row.presence !== "exiting",
        ),
      );
    }, TOPOLOGY_NODE_EXIT_MS);
...
export function LiveTopologyCard({ agents, workspaces }: Props) {
  ...
  const animatedWorkspaces = useAnimatedTopologyItems(
    sortedWorkspaces,
    getWorkspaceTopologyId,
  );
  ...
  const [hoveredItem, setHoveredItem] = useState<HoveredTopologyItem | null>(
    null,
  );
  const animateConnectionMotion =
    connections.length <= TOPOLOGY_MOTION_CONNECTION_LIMIT;
        ...
        <CardHeader>
          <CardHeading>
            ...
            <CardSubtitle>
              ...
              Direct Redis clients may not report sessions and may not appear
              here.
            </CardSubtitle>
          </CardHeading>
        </CardHeader>
