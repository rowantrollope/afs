...
// NodeFlusherWithOwner is an optional Flush variant carrying the kernel's
// POSIX lock owner. It takes precedence over NodeFlusher, allowing a close to
// release this owner's locks even when they were acquired through another FD.
type NodeFlusherWithOwner interface {
	FlushWithOwner(ctx context.Context, f FileHandle, owner uint64) syscall.Errno
}
...
// FileFlusherWithOwner is the file-handle form of NodeFlusherWithOwner.
type FileFlusherWithOwner interface {
	FlushWithOwner(ctx context.Context, owner uint64) syscall.Errno
}
