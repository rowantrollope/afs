...
func callFusermount(mountPoint string, opts *MountOptions) (int, error) {
	bin, err := fusermountBinary()
	if err != nil {
		return -1, err
	}
	return callFusermountWithHelper(mountPoint, opts, bin)
}
func callFusermountWithHelper(mountPoint string, opts *MountOptions, bin string) (fd int, err error) {
	...
	proc := exec.CommandContext(mountContext(opts), bin, cmd[1:]...)
	proc.Env = []string{"_FUSE_COMMFD=3"}
	proc.ExtraFiles = []*os.File{remote}
	proc.Stdout = os.Stdout
	proc.Stderr = os.Stderr
	if err = proc.Run(); err != nil {
		if mountContext(opts).Err() != nil {
			return -1, mountContext(opts).Err()
		}
		return -1, fmt.Errorf("fusermount failed: %w", err)
	}

	fd, err = getConnectionContext(mountContext(opts), local)
	if err != nil {
		return -1, err
	}
