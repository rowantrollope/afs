...
func NewServer(fs RawFileSystem, mountPoint string, opts *MountOptions) (*Server, error) {
	...
	if err := waitMountReadable(mountContext(&o), fd); err != nil {
		syscall.Close(fd)
		return nil, err
	}
...
func (ms *Server) WaitMount() error {
	...
	select {
	case err = <-ms.ready:
	case <-mountContext(ms.opts).Done():
		return mountContext(ms.opts).Err()
	}
	if err != nil {
		return err
	}
	...
	if mountContext(ms.opts).Done() == nil {
		return pollHack(ms.mountPoint)
	}
	done := make(chan error, 1)
	go func() { done <- pollHack(ms.mountPoint) }()
	select {
	case err := <-done:
		return err
	case <-mountContext(ms.opts).Done():
