...
func mount(mountPoint string, opts *MountOptions, ready chan<- error) (fd int, err error) {
	...
	return mountWithHelper(mountPoint, opts, ready, bin)
}
func mountWithHelper(mountPoint string, opts *MountOptions, ready chan<- error, bin string) (fd int, err error) {
	...
	cmd := exec.CommandContext(mountContext(opts), bin,
	...
	go func() {
		...
		waitErr := cmd.Wait()
		if waitErr != nil {
			waitErr = fmt.Errorf("mount_osxfusefs failed: %v. Stderr: %s, Stdout: %s",
				waitErr, errOut.String(), out.String())
		}
